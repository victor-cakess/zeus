...
Infra/platform decisions for the **Zeus** energy data platform. Each entry records the decision, the alternatives considered, why the chosen option won, and known trade-offs.

- **Project overview, diagrams, and how to run it** → [README.md](README.md).
- **Modeling / business-rule decisions** (grain, metric definitions, join semantics) → [`transform/DECISIONS.md`](transform/DECISIONS.md), numbered `M-N` in the same format.
- **Operational commands and per-pipeline detail** (secrets paths, observed performance, troubleshooting) → [RUNBOOK.md](RUNBOOK.md).

Cross-cutting decisions come first; pipeline-specific decisions (EIA / NOAA / FRED / dbt) are grouped per source below.

# Cross-cutting decisions

## 1. AWS region: `sa-east-1` (São Paulo)

**Chosen:** `sa-east-1` — it matches the pre-existing Terraform state backend (`zeus-analytics-tfstate`) and is closest to local development in Brazil. `us-east-1` / `us-west-2` are cheaper, but the cost delta is negligible at these volumes and splitting infra across regions adds operational complexity.
...
**Chosen:** A single `snowflake_warehouse` in `infra/core/` — `x-small` (smallest/cheapest), auto-suspend 60 s, auto-resume, so it only bills while a query runs. It lives in `infra/core/` because a warehouse is account-level shared infrastructure, not pipeline-specific.

**Trade-offs:** it backs all three sources' loads plus the dbt build — each `COPY INTO` and `dbt build` runs on it. A single shared x-small is sufficient for these staggered daily batch loads; a per-workload split would come only if they start contending.
...
- Timeout 300 s: observed daily runs are well under a minute. The 5-minute ceiling is headroom against a slow EIA API.
- Python 3.12 matches the project's `.python-version`.

Observed performance numbers (duration, memory, cold start) live in [RUNBOOK.md](RUNBOOK.md) → EIA daily ingestion.
...
**Trade-offs:**
- The append-only landing table carries ~7× row duplication from the lookback overlap; the dedup is `stg_eia__generation`'s job (keeps the latest `ingestion_date` per `(period, respondent, fueltype)`).
...
**Why overwrite won:** simplest to run with the existing tooling; the landing table is append-only and deduped downstream on `(date, station)` in `stg_noaa__weather`, so re-loading the original 4 BAs as duplicates is tolerated and cleaned in the staging layer.
...
**Trade-offs:** every run re-fetches ~900 observations and appends them to the landing table (~330K rows/year, a few MB compressed; S3 ~50 MB/year) — deliberate duplication, deduped in `stg_fred__prices` on `(series, date)` keeping the latest `ingestion_date`.
...
Runs `dbt build` (staging + intermediate + marts models and their tests — 16 models, 73 tests) over `transform/` against `ZEUS_DEV`, daily, as the `Dbt` step of the state machine (decision 14) — between the ingest Parallel and the digest.
...
**Why:**
- The whole daily build is well under a minute (16 models, 73 tests — duration detail in RUNBOOK) — squarely a Lambda-sized job; the 300 s timeout and 2048 MB leave generous headroom (memory detail in RUNBOOK).
