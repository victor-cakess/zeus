...
Runs `dbt build` (staging + intermediate + marts models and their tests — 9 models, 52 tests) over `transform/` against `ZEUS_DEV`, daily, as the `Dbt` step of the state machine (decision 14) — between the ingest Parallel and the digest.
...
**Why:**
- The whole daily build is ~23 s (9 models, 52 tests) — squarely a Lambda-sized job; the 300 s timeout and 2048 MB leave generous headroom (observed peak ~273 MB).
