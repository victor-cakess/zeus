...
infra/
  ...
  core/                           # Terraform root: shared S3 bucket + SNS alerts topic + Snowflake warehouse/db + per-source Snowflake DDL
    ...
    snowflake_eia_region.tf       # module "eia_region_landing" — EIA region-data (D/DF/NG/TI) integration + ZEUS_DEV.EIA_REGION DDL + loader
    snowflake_eia_interchange.tf  # module "eia_interchange_landing" — EIA interchange-data (BA↔BA flows) integration + ZEUS_DEV.EIA_INTERCHANGE DDL + loader
  ...
  pipelines/
    ...
    eia_region/                   # EIA region-data root: SSM (own snowflake key; CONSUMES eia's api-key path — never re-authors it) + lambda_job; same outputs
    eia_interchange/              # EIA interchange-data root: same consume-not-author key pattern; 81-unit fromba fan-out (the 71 list + 10 interchange-only BAs)
...
src/
  shared/                         # importable by every Lambda; copied into each build at zip root
    ...
    eia_api.py                    # paginated EIA v2 fetch loop shared by the three EIA-family clients (route URL + facet injected)
  ...
  lambdas/
    ...
    eia_region/ingest/            # EIA region-data Lambda: same trio; client.py = region-data route, no type facet (D/DF/NG/TI in one fetch)
    eia_interchange/ingest/       # EIA interchange-data Lambda: same trio; client.py = interchange-data route, fromba facet (all neighbors in one fetch)
...
transform/                        # dbt project (staging + intermediate + marts + reporting models, seeds, tests; profiles.yml env-var driven, key-pair auth) — COPYed into the dbt image at build
  seeds/                          # reference-data CSVs (+schema: seeds — MANDATORY, or the prod Lambda drops them in PUBLIC); first seed: ba_centroids (M-23)
...
backfill/                         # one-off historical backfill scripts (reuse src/ via _bootstrap.py); see backfill/README.md
  ...
  eia_interchange/                # same two-phase pattern (clone of eia_region/, interchange-data route); same 2019-01-01 startPeriod; backfilled 2019→2026
...
| EIA region | `zeus-dev-eia-region-ingest` | `ZEUS_DEV.EIA_REGION.EIA_REGION_GRID` | `(period, respondent, type)`, type ∈ D/DF/NG/TI | 71 BAs (`infra/pipelines/eia_region/locals.tf`, deliberately duplicated) | 7-day | hourly demand/forecast/net-gen/interchange; **shares** EIA's API key (eia root owns the SSM param) |
| EIA interchange | `zeus-dev-eia-interchange-ingest` | `ZEUS_DEV.EIA_INTERCHANGE.EIA_INTERCHANGE_GRID` | `(period, fromba, toba)`, signed MWh (positive = fromba exports) | 81 frombas (`infra/pipelines/eia_interchange/locals.tf` — the 71 list + 10 interchange-only BAs) | 7-day | hourly BA↔BA flows; both directions land (deliberate — the asymmetry test compares them); **shares** EIA's API key |
...
S3 layout is identical across all five (`raw/<source>/.../<unit>.json`, `curated/<source>/.../<source>_grid.parquet`, `reports/<source>/.../run_report.json`). The three EIA-family clients are thin route/facet wrappers over the shared paginated fetch loop in `src/shared/eia_api.py` (extracted at the third client — rule of three).
...
- **Contract:** for each source in `SOURCES` (`eia,eia_region,eia_interchange,noaa,fred`) reads today's run report + the dbt report and publishes **one** combined email to `zeus-dev-alerts`. dbt is **not** a fan-out source, so it stays out of `SOURCES`. **Adding a future source = append it to `var.sources`.** Reuses `src/shared/report.py` (`format_digest`, `format_dbt_section`). **SNS caps subjects at 100 ASCII chars** — the subject is deliberately static; all detail lives in the body.
...
- **Table contract:** append-only landing; duplication from the lookback overlap is deduped downstream in dbt (EIA on `(period, respondent, fueltype)`, EIA region on `(period, respondent, type)`, EIA interchange on `(period, fromba, toba)`, NOAA on `(date, station)`, FRED on `(series, date)`) keeping the latest `ingestion_date`. Per-file load metadata makes same-day re-runs idempotent.
- **Module note:** EIA's landing resources were originally inline in `snowflake_eia.tf`; they were moved into the module via `terraform state mv` (no destroy/recreate — the module reproduces every name/comment exactly). See the ADR.md `snowflake_landing` decision for the move list.
...
### Code & infra
- Lambda source lives in `src/lambdas/<source>/ingest/`. `handler.py` is thin orchestration; the only genuinely source-specific modules are `client.py` (HTTP client) and `schema.py` (pyarrow schema + normalization). Everything else is shared in `src/shared/` (`paths`, `s3_io`, `ssm`, `sns`, `snowflake_io`, `time_window`, `eia_api`, `report`, **`ingest`**) and copied into the build dir at the zip root, so handlers import them as `from shared import …`. `eia_api` owns the EIA v2 paginated fetch loop; the three EIA-family `client.py` files just bind their route URL + facet name to it (rule of three — changing `src/shared/` re-fingerprints **every** ingest Lambda's build, so re-apply the eia/eia_region/eia_interchange roots together after touching it). `ingest.run_ingest(...)` is the shared daily orchestrator (source-specific pieces injected); `report.py` is source-agnostic (takes `source` as a param) and used by both ingest Lambdas and the digest. `snowflake_io` owns the key-pair connection (`copy_into`) and the `COPY INTO` statement builder (`copy_statement`); the daily handlers build SQL via the latter (the backfills keep their own inline statement).
...
### CI (GitHub Actions)
- **Phase 1 (live):** `.github/workflows/ci.yml` runs the offline gates on every PR and push to main — pre-commit (gitleaks), `dbt parse` (dummy creds; parse doesn't connect), the **Lambda unit suite** (`unit-tests` job: `uv sync` + `uv run pytest -q` — fully offline, the tests stub S3/SSM/Snowflake so no creds), and per-root `terraform fmt -check` + `validate` (`init -backend=false` — no state, no cloud creds). It automates the cheap end of "Done means"; the AWS-touching tiers stay local.
