...
- **Trigger:** EventBridge rule `zeus-dev-noaa-daily` fires `cron(30 7 * * ? *)` (staggered 30 min after EIA). It invokes Lambda `zeus-dev-noaa-ingest` directly and asynchronously with `{"units": ["CISO","PJM","ERCO","MISO"]}` (the BA list from `infra/pipelines/noaa/locals.tf`).
...
- **Fan-out unit = BA.** Each BA maps to ~10 weather stations (the `STATIONS` map in `src/lambdas/noaa/ingest/client.py`); the client fetches all of a BA's stations in **one batched NCEI request** (comma-separated `stations=`) and tags every row with its `ba`. Raw layout is one `<ba>.json` per BA, mirroring EIA.
...
# NOTE: the NCEI API is fragile during peak hours — run the NOAA backfill off-peak (night UTC).
...
### Code & infra
- Lambda source lives in `src/lambdas/<source>/ingest/`. `handler.py` is thin orchestration; the only genuinely source-specific modules are `client.py` (HTTP client) and `schema.py` (pyarrow schema + normalization). Everything else is shared in `src/shared/` (`paths`, `s3_io`, `ssm`, `sns`, `snowflake_io`, `time_window`, **`report`**) and copied into the build dir at the zip root, so handlers import them as `from shared import …`. `report.py` is source-agnostic (takes `source` as a param) and used by both ingest Lambdas and the digest. `snowflake_io.copy_into(...)` is source-agnostic too — the caller builds the SQL, so the daily handlers and the backfills share it.
- Per-source handlers are copy-adapted, not shared — ~90% overlaps, but they differ enough (api-key-or-not, fetch signature, hourly vs daily window) that a shared orchestrator would be a leaky abstraction at this scale.
