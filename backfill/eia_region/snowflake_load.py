"""One-time backfill: load every curated EIA region-data Parquet already in S3
into ZEUS_DEV.EIA_REGION.EIA_REGION_GRID with a single whole-stage COPY INTO.

    uv run python snowflake_load.py

This is the same operation as the daily Lambda load with the day-path removed:
the stage covers the whole curated/eia_region/ prefix, and Snowflake's per-file
...
the ZEUS_DEV/EIA_REGION names and only need overriding if you renamed objects.
"""
...
def main():
    ...
    schema = os.environ.get("SNOWFLAKE_SCHEMA", "EIA_REGION")
    table = os.environ.get("SNOWFLAKE_TABLE", "EIA_REGION_GRID")
    stage = os.environ.get("SNOWFLAKE_STAGE", "EIA_REGION_STAGE")
    ...
    rows = snowflake_io.copy_into(
        account=account,
        user=os.environ.get("SNOWFLAKE_USER", "ZEUS_DEV_EIA_REGION_LOADER"),
        private_key_pem=private_key_pem,
        role=os.environ.get("SNOWFLAKE_ROLE", "ZEUS_DEV_EIA_REGION_LOADER_ROLE"),
