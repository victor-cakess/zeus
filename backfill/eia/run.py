"""CLI entry point for the EIA historical backfill.

    uv run python run.py extract   --start 2017-01-01 [--end YYYY-MM-DD] [--concurrency N]
    uv run python run.py transform --start 2017-01-01 [--end YYYY-MM-DD] [--concurrency N]

Config via env: EIA_API_KEY (required for extract), BUCKET (default
zeus-dev-energy-data), SOURCE (default eia).
"""
...
def _parse_args(argv):
    parser = argparse.ArgumentParser(description="EIA historical backfill")
...
def main(argv=None):
    ...
    source = os.environ.get("SOURCE", "eia")
