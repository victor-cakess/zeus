"""One-time backfill: load every curated EIA Parquet already in S3 into
ZEUS_DEV.EIA.EIA_GRID with a single whole-stage COPY INTO.

    uv run python snowflake_load.py

This is the same operation as the daily Lambda load with the day-path removed:
the stage covers the whole curated/eia/ prefix, and Snowflake's per-file load
metadata loads each Parquet exactly once. Reuses the production COPY helper
...
def main():
    ...
    schema = os.environ.get("SNOWFLAKE_SCHEMA", "EIA")
    table = os.environ.get("SNOWFLAKE_TABLE", "EIA_GRID")
    stage = os.environ.get("SNOWFLAKE_STAGE", "EIA_STAGE")
    ...
    rows = snowflake_io.copy_into(
        account=account,
        user=os.environ.get("SNOWFLAKE_USER", "ZEUS_DEV_EIA_LOADER"),
        private_key_pem=private_key_pem,
        role=os.environ.get("SNOWFLAKE_ROLE", "ZEUS_DEV_EIA_LOADER_ROLE"),
