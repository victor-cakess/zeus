"""Phase A: fetch full history per BA (one call per BA-year), split rows by day,
write one raw JSON per (BA, day) — identical layout to the daily pipeline.
...
def _year_window(year: int, start: date, end: date) -> tuple[str, str]:
    """API start/end (YYYY-MM-DDTHH) for `year`, clamped to the overall range."""
    lo = max(date(year, 1, 1), start)
    hi = min(date(year, 12, 31), end)
    return f"{lo.isoformat()}T00", f"{hi.isoformat()}T23"


def _extract_unit_year(unit, year, window, existing, bucket, source, api_key, logger) -> Counter:
    start, end = window
    print(f"  processing {unit} {year}...", flush=True)
    rows = fetch_with_retry(unit, start, end, api_key)
    if not rows:
    ...
    for row in rows:
        by_day[date.fromisoformat(row["period"][:10])].append(row)
...
def run(units, start: date, end: date, bucket, source, api_key, concurrency, logger):
    overall = Counter()
    for year in range(start.year, end.year + 1):
        ...
        with ThreadPoolExecutor(max_workers=concurrency) as pool:
            futures = {
                pool.submit(
                    _extract_unit_year,
                    unit, year, window, existing, bucket, source, api_key, logger,
                ): unit
                for unit in units
            }
