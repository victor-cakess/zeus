"""Phase C: load every curated FRED Parquet in S3 into ZEUS_DEV.FRED.FRED_GRID with
a single whole-stage COPY INTO. Run after extract + transform have landed
curated/fred/.

    SNOWFLAKE_ACCOUNT=<org-account> SNOWFLAKE_PRIVATE_KEY_FILE=sf_fred_loader.p8 \\
        uv run python snowflake_load.py

Same operation as the daily Lambda load with the day-path removed: the stage covers
the whole curated/fred/ prefix, and Snowflake's per-file load metadata loads each
Parquet exactly once (the daily pipeline's already-loaded files are skipped). Reuses
the production COPY helper (src/shared/snowflake_io.py).
...
path to the loader .p8). The rest default to the ZEUS_DEV/FRED names.
"""
...
def main():
    ...
    schema = os.environ.get("SNOWFLAKE_SCHEMA", "FRED")
    table = os.environ.get("SNOWFLAKE_TABLE", "FRED_GRID")
    stage = os.environ.get("SNOWFLAKE_STAGE", "FRED_STAGE")
    ...
    rows = snowflake_io.copy_into(
        account=account,
        user=os.environ.get("SNOWFLAKE_USER", "ZEUS_DEV_FRED_LOADER"),
        private_key_pem=private_key_pem,
        role=os.environ.get("SNOWFLAKE_ROLE", "ZEUS_DEV_FRED_LOADER_ROLE"),
