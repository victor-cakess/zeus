locals {
  ...
  ingest_name = "${local.prefix}-eia-interchange-ingest"
  ...
  snowflake_key_ssm_path = "/${local.project}/${local.env}/snowflake/eia_interchange_loader_private_key"
  ...
  raw_arn_pattern     = "${local.bucket_arn}/raw/eia_interchange/*"
  curated_arn_pattern = "${local.bucket_arn}/curated/eia_interchange/*"
  reports_arn_pattern = "${local.bucket_arn}/reports/eia_interchange/*"
}
...
# put-parameter`. The matching public key lives on the ZEUS_DEV_EIA_INTERCHANGE_LOADER user.
resource "aws_ssm_parameter" "snowflake_key" {
  name        = local.snowflake_key_ssm_path
  description = "ZEUS_DEV_EIA_INTERCHANGE_LOADER RSA private key. Set out-of-band via aws ssm put-parameter."
  type        = "SecureString"
...
# Single ingest Lambda: fetches every BA in parallel (one request-loop per fromba
# returns signed flows to all of that BA's neighbors), drops raw JSON, then
...
module "ingest" {
  ...
  src_dir         = "${path.module}/../../../src/lambdas/eia_interchange/ingest"
  ...
  env_vars = {
    ...
    SNOWFLAKE_USER                 = "ZEUS_DEV_EIA_INTERCHANGE_LOADER"
    SNOWFLAKE_ROLE                 = "ZEUS_DEV_EIA_INTERCHANGE_LOADER_ROLE"
    ...
    SNOWFLAKE_SCHEMA               = "EIA_INTERCHANGE"
    SNOWFLAKE_TABLE                = "EIA_INTERCHANGE_GRID"
    SNOWFLAKE_STAGE                = "EIA_INTERCHANGE_STAGE"
  ...
  policy_statements = [
    ...
    },
    {
      ...
      Condition = {
        StringLike = {
          "s3:prefix" = ["raw/eia_interchange/*", "reports/eia_interchange/*"]
        }
      }
    },
    {
